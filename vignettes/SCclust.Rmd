...
prior to copy-number profile segmentationsegmenting.

To identify centromeric regions we use `calc_centroareas` function:
...
The location of centromeric regions is now tabulated in `centroareas`.
...
Using centromeric regions tabulated in `cytoBand.txt` we determine which bins are inside
or intersect with centromeric regions using `calc_regions2bins` function:
...
After excluding centromeric bins from the binning scheme, the new binning scheme has smaller number of bins:
...
The function `varbin_input_files` helps us to collect varbin files for all samples in our `T10data/varbin`
directory:
...
Next, we convert the set of segmented copy number profiles into a feature matrix. This process includes a number of steps.
```{r}
