...
\todo{Right now we mask p11 through q11. Ideally we should make this a parameter in the config file.}
...
value for three alternative definitions of ploidy: the genome-wide median, the genome-wide mode and the mode of all chromosome-wide
modes. Also, at this point we compute the percentage of the genome with copy number 0, i.e., lost homozygously. A high percentage of
homozygous loss is likely a sign of DNA degradation ex-vivo. Such cells are removed from further processing (this is controlled using 
`homoloss` parameter of `calc_pinmat` fuction).
\todo{Display the ploidies table here?}
...
Also, since hc is the only argument, it is hard for the user to see the relation between these parameters and how the clones 
are computed.}
