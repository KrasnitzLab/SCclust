...
\todo{some minimal info on the varbin format}
...
position of the bin in the chromosome, absolute position of the bin, how many reads are mapped in this bin and the ratio.
...
Segmentation (CBS) algorithm, as implemented in the R package `DNAcopy`, but 
modified for a more consistent elimination of very short segments.

\todo{mention the quantal procedure}
```{r}
...
\todo{Where do we remove shredded cells? ploidies.txt? homoloss?}
...
segments may merge as a result. As part of this procedure we calculate the ploidity and homoloss of each cell and remove the cells with
high homoloss (this could be controlled using `homoloss` parameter of `calc_pinmat` fuction).
...
\todo{Where are the parameters such as max FDR and the number of shared features defined? 
Also, since hc is the only argument, it is hard for the user to see the relation between these parameters and how the clones are computed.}
...
The suction `find_clones` supports a lot of parameters that control its behavior such as 
...
To complete the analysis, we compute subclones. 
```{r subclones}
subclones <- find_subclones(hc, pinmat_df, pins_df, nsim=nsim)
```

For the full list of parameters that control the behavior of `find_sublones` consult the manual pages of SCclust package.
