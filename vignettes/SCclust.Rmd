...
\todo{What are the arguments of hclust_tree?}
...
with the feature. As an interface with Python software, the function `tree_py` is provided, which outputs the same hierarchical tree in the format for Python hierarchical clustering suite.
```{r}
...
\todo{Where are the parameters such as max FDR and the number of shared features defined? Also, since hc is the only argument, it is hard for the user to see the relation between these parameters and how the clones are computed.}
