...
cbs.segment01 <- function(indir, outdir, varbin.gc, varbin.data, sample.name, alt.sample.name, alpha, nperm, undo.SD, min.width) {
	...
	bad <- read.table("/mnt/wigclust5/data/safe/kendall/badbins01/hg19.badbins.20k.txt", header=F, as.is=T, stringsAsFactors=F)
	...
	write.table(thisShort, sep="\t", file=paste(outdir, "/", sample.name, ".hg19.20k.k50.nobad.varbin.short.cbs.txt", sep=""), quote=F, row.names=F) 
	...
	write.table(thisQuantalStatsNobad, sep="\t", file=paste(outdir, "/", sample.name, ".hg19.20k.k50.nobad.varbin.quantal.stats.txt", sep=""), quote=F, row.names=F) 
...
cbs.segment01(indir="/mnt/wigstore3/user/alexande/nyu003/hg19.50k/processed", outdir="/mnt/wigstore3/user/alexande/nyu003/hg19.50k/processed.v2", varbin.gc="/mnt/wigclust5/data/safe/kendall/sequences/varbin.gc.content.20k.bowtie.k50.hg19.txt", varbin.data="CJA3261.varbin.20k.txt", sample.name="CJA3261", alt.sample.name="FC64BEMAAXX lane 6 JM510 A10 bc68 NYU_003_12_PBXW0039 2C", alpha=0.05, nperm=1000, undo.SD=1.0, min.width=5)
