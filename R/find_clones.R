...
#'@param fdrthresh maximal allowed value for log10(FDR) for any pair of leaves in a clone node. Default: -2.
#'@param sharemin A feature is considered 'widely shared' if present in sharemin fraction of leaves in a node.Default: 0.90.
#'@param nshare Minimal number of 'widely shared' features in a hard clone. Default: 3.
#'@param bymax Logical. If TRUE (default), use maximal, and otherwise mean, FDR for the node as a criterion for a hard clone.
#'@param climbfromsize An integer: minimal size of a hard clone allowed to be expanded 
#'@param climbtoshare An integer: expand the clone as long as the number of widely shared features is at least this value
