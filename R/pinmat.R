...
calc_segments_short <- function(gc_df, segment_df, homoloss=1.0, chromrange=1:24) {
  ...
  ploidies_df <- calc_ploidies_internal(gc_df, segment_df, a, chromrange=chromrange)
...
calc_ploidies_internal <- function(gc_df, segment_df, rounded_df=NULL, chromrange=1:24) {
...
calc_smear_breakpoints <- function(
    short_df, censored=NULL, smear=1, keepboundaries=F, chromrange=1:24) {
...
#' Calculates ploidies for samples passed in \code{segment_df}.
#' 
...
#' @return ploidies calculated for samples in \code{segment_df}.
#' @export
calc_ploidies <- function(gc_df, segment_df, chromrange=1:24) {
  augment_df <- augment_gc(gc_df, segment_df)
  ploidies_df <- calc_ploidies_internal(
      augment_df, segment_df, chromrange=chromrange)
  return(ploidies_df)
}
