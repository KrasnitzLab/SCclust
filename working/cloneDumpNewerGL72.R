...
case_dir <- file.path(data_dir, "gleason7.2")
assertthat::assert_that(file.exists(case_dir))
...
load(paste(rdir,prostate,"smear1bpLog10FisherHC",hctype,".rda",sep=""))
...
pinmat<-
	as.matrix(read.table(paste(rdir,prostate,"smear1bpPinMat.txt",sep=""),header=T,as.is=T))
pins<-read.table(paste(rdir,prostate,"smear1bpPins.txt",sep=""),header=T,as.is=T)
...
#find the clone sub-matrix of the "pinmat' and the corresponding pins
	...
	clonedir<-paste(outdir,"clone",hctype,as.character(clone),".",Sys.Date(),".",
