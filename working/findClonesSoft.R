...
source("./TreePy.R")
cellnames<-colnames(read.table(paste(hcdir,prostate,"smear1bpPinMat.txt",sep=""),header=T))
jguide<-read.table("./annot/joan02.guide111114.txt",
...
#FDR is computed by comparing true to simulated CDF. However, empirical true CDF is only defined (False Discovery Rate - FDR)
...
dim(mfdr)
colnames(mfdr) <- cellnames
rownames(mfdr) <- cellnames
